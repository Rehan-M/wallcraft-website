...
// ✅ CORS for all routes (your domains + local dev)
app.use(
  cors({
    origin: [
...
app.post("/api/contact", async (req, res) => {
  ...
  try {
    ...
    if (!process.env.EMAIL_FROM || !process.env.EMAIL_TO) {
      console.error("❌ EMAIL_FROM or EMAIL_TO is not set in environment");
      return res
        .status(500)
        .json({ error: "Email configuration error on server." });
    }
    ...
    // 🔍 Log who we're emailing (for debugging)
